...
{
    public partial class frmQuanLyCaNhan : FormBaseGarco10
    {
        public frmQuanLyCaNhan(int m_iID_TaiKhoanDangNhap)
        {
            InitializeComponent();
        ...
        private void DoiMauTrangThai(ucFlexGrid fg)
        {
            for (int i = fg.Rows.Fixed; i < fg.Rows.Count; i++)
            {
                CellRange rg = fg.GetCellRange(i, fg.Cols["Ten_TrangThai"].Index);
                if (fg.GetDataDisplay(i, "ID_TrangThai").ToString().Trim() == "")
                {
                    rg.Style = cs;
                }
                else
                    
                    //switch ((HRM_Enumerations.HopDong_QuyetDinh_TrangThai)int.Parse(fg.GetDataDisplay(i, "TrangThai")))
                    switch (fg.GetDataDisplay(i, "ID_TrangThai").ToString().Trim())
                    {
                        case "1":
                            rg.Style = csChoXuLy;
                            break;
